...
function describeNewsError(err: unknown, lang: Language): string {
  const en = lang === "en";
  if (err instanceof ApiError) {
    switch (err.kind) {
      case "http":
        return en
          ? `Failed to load article (HTTP ${err.status ?? ""})`.trimEnd().replace(/\($/, "")
          : `Gagal memuat berita (HTTP ${err.status ?? ""})`.trimEnd().replace(/\($/, "");
      case "invalid-json":
        return en ? "Unexpected article format" : "Format data berita tidak sesuai";
      case "network":
      default:
        return en
          ? "Failed to reach Banyuwangi Tourism server"
          : "Gagal menghubungi server Banyuwangi Tourism";
    }
  }
  return en
    ? "Failed to reach Banyuwangi Tourism server"
...
export function NewsDetailModal({ slug, initialData, isOpen, onClose }: NewsDetailModalProps) {
  ...
  useEffect(() => {
    ...
    const fetchArticleDetail = async () => {
      ...
      try {
        const article = await fetchNewsArticleDetail(slug);
        if (isMounted) {
          setDetailArticle(article);
        }
      } catch (err: unknown) {
        if (isMounted) {
          setError(describeNewsError(err, lang));
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  const getImgUrl = (foto?: string) => getNewsImageUrl(foto);
